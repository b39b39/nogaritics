...
import { deleteTrack } from "@/app/actions/content";
...
export function TrackActions({ trackId, canEdit, initialStarred, isLoggedIn }: Props) {
  ...
  function handleStar() {
    ...
    startStarTransition(async () => {
      await toggleStarred(trackId, "track", next);
      router.refresh();
    });
  }
  ...
  return (
    <div className="flex flex-col items-end justify-between flex-shrink-0 self-stretch">
        ...
        <StarIcon
          className={`w-6 h-6 transition-colors ${
            starred
              ...
              : "fill-none text-white/70 stroke-[1.5]"
          }`}
        />
      </button>
      ...
      <ConfirmModal
        open={deleteOpen}
        title="트랙을 삭제하시겠습니까?"
        description="삭제한 트랙은 복구할 수 없습니다."
        confirmLabel="삭제"
        danger
