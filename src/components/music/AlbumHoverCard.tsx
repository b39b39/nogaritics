...
const previewCache = new Map<string, AlbumPreview>();
...
function Card({
  ...
  return (
    ...
    >
      {data.image ? (
        <Image src={data.image} alt="" fill className="object-cover" sizes="200px" />
      ) : (
...
export function AlbumHoverCard({ albumId, children }: Props) {
  ...
  function handleEnter() {
    ...
    showTimer.current = setTimeout(async () => {
      ...
      let data = previewCache.get(albumId);
      if (!data) {
        try {
          const res = await fetch(`/api/albums/${albumId}`);
          if (res.ok) {
            data = await res.json() as AlbumPreview;
