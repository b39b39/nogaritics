...
import { itunesSearchArtists, type ItunesArtist } from "@/lib/itunes";
...
export function ArtistPostClient({ countries }: Props) {
  ...
  const [itunesArtistId, setItunesArtistId] = useState<string | null>(null);
  const [links, setLinks] = useState<Record<string, string>>({
    xUrl: "", instagramUrl: "", youtubeUrl: "", youtubeMusicUrl: "", appleMusicUrl: "", soundcloudUrl: "",
  });
  ...
  function onItunesSelect(artist: ItunesArtist) {
    setName(artist.artistName);
    setItunesArtistId(String(artist.artistId));
    setLinks((prev) => ({ ...prev, appleMusicUrl: artist.artistViewUrl ?? "" }));
  }
    ...
    try {
      const resolvedIds = await Promise.all(
        members.map(async (m) => {
          ...
          if (!r.ok) throw new Error(`멤버 "${m.name}" 등록 실패`);
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        <div className="rounded-2xl overflow-hidden">
          ...
          <div
            ...
            title="배너 이미지 변경"
          >
            {banner && <Image src={banner} alt="" fill className="object-cover" />}
      ...
      {/* ── iTunes 검색 ──────────────────────────────────────────────── */}
      <ItunesSearch<ItunesArtist>
        placeholder="iTunes에서 아티스트 검색…"
        onSearch={itunesSearchArtists}
        renderResult={(a) => (
          <div className="px-3 py-2.5">
            <span className="text-sm font-medium text-gray-900">{a.artistName}</span>
          </div>
        )}
        onSelect={onItunesSelect}
      />
      ...
      {isGroup && (
        <div className="space-y-2">
          ...
          {members.length > 0 ? (
            <div className="space-y-1.5">
              {members.map((m) => (
                <div key={m.tempKey} className="flex items-center gap-2.5 bg-gray-50 rounded-lg px-3 py-2 border border-gray-100">
      ...
      <ArtistSearchModal
        ...
        onSelect={addMember}
        excludeIds={members.filter((m) => m.id !== null).map((m) => m.id as string)}
      />
    </div>
  );
}
