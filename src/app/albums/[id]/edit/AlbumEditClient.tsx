...
import { updateAlbum, type UpdateAlbumTrackEntry } from "@/app/actions/update";
...
let uidCounter = 0;
function nextUid() {
  return `track-${++uidCounter}`;
}

export function AlbumEditClient({ initialData }: { initialData: InitialData }) {
  ...
  const [tracks, setTracks] = useState<TrackItem[]>(() => {
    const sorted = [...initialData.tracks].sort((a, b) => (a.trackNumber ?? 9999) - (b.trackNumber ?? 9999));
    return sorted.map((t) => ({
      uid: t.id,
      id: t.id,
      name: t.name,
      image: t.image,
      artistDisplay: t.artists.map((a) => a.artist.name).join(", "),
    }));
  });
  ...
  function handleRemoveRequest(uid: string) {
    setRemovePending(uid);
  }

  function handleRemoveConfirm() {
    if (removePending) {
      setTracks((items) => items.filter((t) => t.uid !== removePending));
    }
  ...
  function handleTrackAdd(selected: SelectedTrack) {
    ...
    if (tracks.some((t) => t.uid === uid || (selected.id && t.id === selected.id))) return;
    setTracks((prev) => [
      ...prev,
      {
        uid,
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    startTransition(async () => {
      const trackEntries: UpdateAlbumTrackEntry[] = tracks.map((t, i) => ({
        id: t.id,
        trackNumber: i + 1,
        itunesTrackId: t.itunesTrackId,
        name: t.name,
        image: t.image,
        itunesArtistId: t.itunesArtistId,
        artistName: t.artistDisplay,
        appleMusicUrl: t.appleMusicUrl,
      }));

      const result = await updateAlbum(initialData.id, {
        name: name.trim(),
      ...
      if (result.ok) {
        router.push(`/albums/${initialData.id}`);
      } else {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8">
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-2xl border border-gray-100 p-6 space-y-6">
        <h2 className="text-lg font-bold text-gray-900">앨범 수정</h2>
        ...
        <div className="flex gap-3 pt-2">
          <button
            type="button"
            onClick={() => router.push(`/albums/${initialData.id}`)}
